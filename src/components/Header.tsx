    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <Link to="/order-status" className="text-gray-700 hover:text-blue-500 transition-colors font-medium">
            訂單查詢
          </Link>
        </nav>
        ...
        <div className="flex items-center space-x-3">
          ...
          <Link to="/cart">
            <Button variant="outline" size="sm" className="relative border-gray-200 hover:border-blue-500">
              <ShoppingCart className="w-4 h-4 mr-2" />
              購物車
              {itemCount > 0 && (
                <Badge 
                  className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-blue-500 text-white border-0"
                >
                  {itemCount}
                </Badge>
              )}
